...
static void SetValue(Value *V, GenericValue Val, ExecutionContext &SF) {
    if (nitrous_verbosity >= NITROUS_VERBOSITY_INTERPRETING) {
        if (GVTOP(Val))
            outs() << "Value: " << GVTOP(Val) << '\n';
        else
            outs() << "Value: " << Val.IntVal.getSExtValue() << '\n';
    }
  SF.Values[V] = Val;
}
...
void Interpreter::visitCallSite(CallSite CS) {
  ...
  if (F && F->isDeclaration())
    ...
    // We dont't want to lower memset and memcpy for perf reasons.
    case Intrinsic::memcpy:
      func_addr = (uint64_t)memcpy;
      break;
    case Intrinsic::memset:
      func_addr = (uint64_t)memset;
      break;
